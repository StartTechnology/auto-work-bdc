...
export const BusinessConfig = defineStore(
  ...
  () => {
    ...
    //ocr消息
    const OCRMessage = ref([] as string[]);
    ...
    //请求URL
    const get_loan_type_url = computed(() => {
      return pageConfig.server_url + "/setting/getloantype";
    });
    const get_loan_guarantee_url = computed(() => {
      return pageConfig.server_url + "/setting/getguaranteescope";
    });
    const get_loan_manager_url = computed(() => {
      return pageConfig.server_url + "/setting/getcustomermanager";
    });
    const get_developer_info_url = computed(() => {
      return pageConfig.server_url + "/setting/getdevelopers";
    });
    //提交任务URl
    const submit_task_url = computed(() => {
      return pageConfig.server_url + "/pushtask/pushtask";
    });
    //写入数据url
    const write_data_url = computed(() => {
      return pageConfig.server_url + "/pushtask/towps";
    });
    ...
    return {
      ...
      get_loan_type_url,
      get_loan_guarantee_url,
      get_loan_manager_url,
      get_developer_info_url,
      submit_task_url,
      write_data_url,
