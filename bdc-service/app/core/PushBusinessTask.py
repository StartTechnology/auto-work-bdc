...
class BusinessInfo(BaseModel):
    ...
    loan_manger:str|None=None
    loan_amount:int|None=None
    loan_contract:str|None=None
    loan_tream:dict|None=None
    loan_guarantee:str|None=None

    certificate:list[str]|None=None
    #义务人开发商信息
    obligor:dict|None=None
    #购房备案合同号
...
#网页操作 - 选择业务类型并进入
async def webSelectType(business_type:str,certificate:list[str]|None=None):
    new_certificate=[]
    if certificate:
    ...
    for img_dir in img_path.iterdir():
        if img_dir.is_dir():
            ...
            if img_dir.name=="不动产登记申请书":
                ...
                ele_annex_box.ele('tag=li@@text():关于抵押预告登记的约定',timeout=3).ele('tag=div@@class:el-upload--text',timeout=3).click.to_upload('\n'.join(img_list))
